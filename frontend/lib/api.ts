...
// Strip client-only fields (unit, packWeightGrams, sku) — backend DTO has forbidNonWhitelisted: true.
function toDtoItem(item: CartItem) {
  return {
...
export const api = {
  ...
  preview: (currency: string, items: CartItem[], expoDiscountEnabled: boolean) =>
    request<PricePreview>('/orders/preview', { method: 'POST', body: JSON.stringify({ currency, items: items.map(toDtoItem), expoDiscountEnabled }) }),
  intake: (key: string, currency: string, items: CartItem[], customerName: string, customerContact: string, expoDiscountEnabled: boolean) =>
    request<{ operation: { id: string } }>('/checkout-intake', { method: 'POST', headers: { 'Idempotency-Key': key }, body: JSON.stringify({ currency, items: items.map(toDtoItem), customerName, customerContact, expoDiscountEnabled }) }),
