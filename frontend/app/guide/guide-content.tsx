...
const guideCopy: Record<Locale, GuideCopy> = {
  en: {
    ...
    whatDemonstrated: 'What is demonstrated', builtWith: 'Built with', stack: [['Application', 'Next.js · React · TypeScript'], ['Backend', 'NestJS · Node.js · TypeScript'], ['Data', 'SQLite · WAL · migrations'], ['Payments', 'Stripe Checkout · webhooks'], ['Deployment', 'Docker Compose · Nginx'], ['Quality', 'GitHub Actions CI']], designedAround: 'Designed around reliable checkout.', projectNotes: 'Project notes',
  ...
  'zh-CN': {
    ...
    whatDemonstrated: '演示内容', builtWith: '技术栈', stack: [['应用', 'Next.js · React · TypeScript'], ['后台', 'NestJS · Node.js · TypeScript'], ['数据', 'SQLite · WAL · 数据库迁移'], ['支付', 'Stripe Checkout · webhook'], ['部署', 'Docker Compose · Nginx'], ['质量', 'GitHub Actions CI']], designedAround: '围绕可靠结账而设计。', projectNotes: '项目说明', notesTitle: '表面简单，底层严谨。', notesBody: 'USD 是 Stripe 的支付金额。CNY 是供应价格表中的参考价，不是汇率换算。应用设计为单个 SQLite 部署，且支付服务调用在数据库事务之外执行。', github: 'GitHub 仓库', idempotency: '支付幂等设计', protectedCheckout: '受保护的在线结账', footer: '展会柜台结账 · 项目指南',
    features: [
...
export default function GuideContent({ checkoutUrl, videoUrl }: GuideContentProps) {
  ...
  return (
    <main className="demo-shell">
      ...
      <section className="demo-stack-section" aria-labelledby="built-with-heading"><div className="demo-section-heading compact"><h2 id="built-with-heading">{text.builtWith}</h2></div><div className="demo-stack-grid">{text.stack.map(([label, value]) => <article className="demo-stack-card" key={label}><span>{label}</span><strong>{value}</strong></article>)}</div></section>
