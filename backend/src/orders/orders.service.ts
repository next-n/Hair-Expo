...
export class OrdersService {
  ...
  async refresh(orderId: string) {
    const row = this.database.connection.prepare('SELECT stripe_payment_link_id FROM orders WHERE id = ?').get(orderId) as { stripe_payment_link_id: string | null } | undefined;
